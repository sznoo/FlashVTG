            ...
            if "relevant_windows" in meta:  ## For Qvhighlights test set
                ...
                if self.dset_name in [
                    ...
                    (
                        ...
                        meta["relevant_windows"][0], meta["duration"], ctx_l
                    )  # only one gt
                    ...
                    (
                        ...
                        meta["relevant_windows"][0], meta["duration"], ctx_l, 2
                    )  # only one gt
    ...
    for k in model_inputs_keys:
        ...
        if k == "saliency_all_labels":
            pad_data, mask_data = pad_sequences_1d(
                [e[1][k] for e in batch], dtype=np.float32, fixed_length=None
            )
...
def prepare_batch_inputs(batched_model_inputs, device, non_blocking=False):
    ...
    if "saliency_all_labels" in batched_model_inputs:
        ...
        targets["relevant_clips"] = batched_model_inputs["saliency_all_labels"].to(
            device, non_blocking=non_blocking
        )
